...
def preprocess_raw_eeg(raw_eeg):
    ...
    Before manipulating a EEG file you have to set the montage using set_montage( ) where the montage
    is of the mne channels type.
    """
...
def extract_and_modify_events(raw_eeg):
    ...
    Here target ID's are the ones where the letter on the screen is matching the target
    letter. We loop through all the events and match to a new dictionary dictating wether its
    frequent rare, rare frequent, or frequent frequent. 
    """
    ...
    for i in range(len(events)):
        current_id = events[i, 2] #array indexing to take out 3rd index which contains the current ID type
        if current_id in target_ids:
            events[i, 2] = 100  # 'frequent_rare' -- all of these set it equal to the new shema of 100, 200, 300
...
for condition in conditions:  
    #have to use np.zeros to initialize a row of zeros to be able to use vstack otherwise wont work 
    ...
    #now delete the intial row sow that just have the pre and post data as rows for each particpant 
    ...
    # Take the average across particpants (axis 0 since participants in rows) to build plots in gyurkovics
...
frequent_rare_array = np.array([])
rare_frequent_array = np.array([])
frequent_frequent_array = np.array([])


for participant in y_frequencies_dict:
    for condition in y_frequencies_dict[participant]:
        if condition == 'frequent_rare':
            frequent_rare_array = np.append(frequent_rare_array, y_frequencies_dict[participant][condition]['yf_avg_pre']['aps'][0])
        elif condition == 'rare_frequent':
            rare_frequent_array = np.append(rare_frequent_array, y_frequencies_dict[participant][condition]['yf_avg_post']['aps'][0])
        else:
            frequent_frequent_array = np.append(frequent_frequent_array, y_frequencies_dict[participant][condition]['yf_avg_pre']['aps'][0])
